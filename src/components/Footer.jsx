  ...
  return (
    <footer className='relative overflow-hidden bg-gray-50 text-gray-800'>
      ...
      <motion.div
        ...
        <motion.div variants={item}>
          ...
          <div className='flex justify-center md:justify-start gap-6 mt-6 text-2xl'>
            <a
              href='https://www.linkedin.com/in/stephen-opeyemi-aderinto/'
              target='__blank'>
              <FaLinkedin className='hover:text-blue-500 transition cursor-pointer' />
            </a>
            <a href='https://x.com/AderintoStephen' target='__blank'>
      ...
      <div className='fixed flex justify-end items-end gap-1 bottom-6 left-6 z-50 group'>
        <a
          href='https://wa.me/447466619073'
          target='_blank'
          rel='noopener noreferrer'
          className='w-full p-4 pr-0  text-green-600 transition'>
          <FaWhatsapp className='text-5xl max-md:text-3xl' />
        </a>
      ...
      {showScroll && (
        <button
          onClick={scrollToTop}
          className='fixed bottom-6 right-6 z-50 bg-blue-700 hover:bg-blue-600 text-white p-4 rounded-full shadow-2xl transition max-md:text-[15px]'>
          <FaArrowUp />
        </button>
      )}
    </footer>
